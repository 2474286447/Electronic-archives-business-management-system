...
export const configCenterApi = {
  fonds: {
    page: (params?: PageQuery) => http.get('/archive/fonds/page', { params }) as unknown as Promise<ApiResult<PageData>>,
    list: () => http.get('/archive/fonds/list') as unknown as Promise<ApiResult<AnyRecord[]>>,
    detail: (id: string | number) => http.get(`/archive/fonds/${id}`) as unknown as Promise<ApiResult<AnyRecord>>,
    byCode: (fondsCode: string) => http.get(`/archive/fonds/code/${fondsCode}`) as unknown as Promise<ApiResult<AnyRecord>>,
    create: (data: AnyRecord) => http.post('/archive/fonds', cleanPayload(data)) as unknown as Promise<ApiResult>,
    update: (id: string | number, data: AnyRecord) => http.put(`/archive/fonds/${id}`, cleanPayload(data)) as unknown as Promise<ApiResult>,
    remove: (id: string | number) => http.delete(`/archive/fonds/${id}`) as unknown as Promise<ApiResult>,
    batchRemove: (ids: Array<string | number>) => http.delete('/archive/fonds/batch', { data: ids }) as unknown as Promise<ApiResult>
  },
  archiveCategory: {
